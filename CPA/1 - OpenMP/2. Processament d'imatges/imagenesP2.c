...
/**** START HERE ****/
  ...
  for (p = 0; p < pasos; p++) {
    //# pragma omp parallel for private(tot, resultado,v,j,k,l)
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        resultado.r = 0;
        resultado.g = 0;
        ...
        // Variables aux
        int 	rAux = resultado.r, 
        	gAux = resultado.g, 
        	bAux = resultado.b;
        
        # pragma omp parallel for private(rAux,gAux,bAux,l,v) reduction(+:tot)
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
          for (l = max(0, j - radio); l <= min(m - 1, j + radio); l++) {
            v = ppdBloque[k - i + radio][l - j + radio];
            rAux  += ppsImagenOrg[k][l].r * v;
            gAux += ppsImagenOrg[k][l].g * v;
            bAux += ppsImagenOrg[k][l].b * v;
            tot += v;
          }
        }
        rAux /= tot;
        gAux /= tot;
        bAux /= tot;
        ppsImagenDst[i][j].r = rAux;
        ppsImagenDst[i][j].g = gAux;
        ppsImagenDst[i][j].b = bAux;
      }
    }
    if (p+1 < pasos)
